...
public class FoodTruckApp {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < trucks.length; i++) {
			trucks[i] = truckApp.getTruckInfoFromUser(sc);
			if (trucks[i] == null) {
				break;
			}
...
//			System.out.println(trucks[i]); //Used for testing userInput. 
		...
		while (truckApp.keepGoing) {
			truckApp.printMenu(sc, truckApp, trucks);
		}
		sc.close();
	}
	...
	private void getHighestRatedTruck(FoodTruck[] trucks) {
		...
		for (int i = 0; i < trucks.length; i++) {
			if (trucks[i] != null) {
				if (trucks[i].getTruckRating() > highestRating) {
					highestRating = trucks[i].getTruckRating();
					index = i;
				}
			}
		}
	...
	private double getAverageFoodTruckRating(FoodTruck[] trucks) { // TODO: Return the avg ratings to user
		...
		for (int i = 0; i < trucks.length; i++) {
			if (trucks[i] != null) {
				double foodRating = trucks[i].getTruckRating();
